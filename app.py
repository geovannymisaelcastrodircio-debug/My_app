...
# ======================= Función de búsqueda universal (corregida) =======================
def universal_search(term, carreras, db):
    ...
    for carrera in carreras:
        ...
        # Campos que queremos buscar (asegúrate que coincidan con tus documentos)
        ors = [
            {"NOMBRE (S)": regex_expr},
            {"A. PAT": regex_expr},
            {"A. MAT": regex_expr},
            ...
            {"REVISOR": regex_expr},
            {"NUM. CONTROL": regex_expr},
        ]

        # Si el término es numérico, intentar búsqueda exacta por número también
        if is_number:
            try:
        ...
        # Añadir nombre de carrera para referencia y evitar colisiones al mostrar
        for d in docs:
            d["_carrera"] = carrera

        resultados.extend(docs)

    # Eliminar duplicados por (NUM. CONTROL, _carrera) si existen
    vistos = set()
    unicos = []
    for r in resultados:
...
if menu == "🔍 Búsqueda universal":
    ...
    if busqueda:
        ...
        if resultados:
            ...
            # Mostrar columnas importantes al inicio si existen
